...
# What's different between v2 and v1? v2 join action extracting and tree converting together to get a most correct error type classify result.
...

{
    ...
    if [ $# -lt 1 ]; then
        echo "Error: Expected at least non-option arguments (parallel_files)" >&2
        show_help
        exit 1
    fi
    ...
    # check errant first since it need to install a older spacy version
    check_python_package "errant"
    check_python_package "spacy"
    if python -c "import spacy; exit(spacy.load('en') is not None)"; then
        echo "  - 'en' for 'spacy' package is not installed. Installing it now..."
        if python -m spacy download en; then
            echo "Download and install 'en' for 'spacy' is successful"
        else
            echo "Error: failed to download and install." >&2
            exit 1
        fi
    else
        echo "  - 'en' for 'spacy' is already installed."
    fi
    ...
    for file in "${parallel_files[@]}"; do
        echo "basename $file"
        python -u seq2seq.py predict -d "$devices" -c "$config" -p "$gec_path" --data "$file" --pred "$tmp_path"/"$(basename "$file")".pred --cache --batch-size="$batch" --beam-size="$topk" --topk="$topk"
        # compose predicted file to original file
        python scripts/generate_eng_treebank/compose_predict_file.py --gold-file "$file" --pred-file "$tmp_path"/"$(basename "$file")".pred --output-file "$tmp_path"/"$(basename "$file")".parallel
    done
    ...
    for file in "${parallel_files[@]}"; do
        ...
        python scripts/generate_eng_treebank/split_instances.py --seed "$seed" --input-file "$tmp_path"/"$(basename "$file")".parallel --output-file "$tmp_path"/"$(basename "$file")"
    ...
    for file in "${parallel_files[@]}"; do
        ...
        python scripts/generate_eng_treebank/expand_correct_lines.py --input-file "$tmp_path"/"$(basename "$file")".correct --match-file "$tmp_path"/"$(basename "$file")".match --output-file "$tmp_path"/"$(basename "$file")".expanded.correct
        # by default we set processes to 8 and chunksize to 64
        python scripts/generate_eng_treebank/convert_tree.py --processes 20 --tokenizer "facebook/bart-large" --source-file "$tmp_path"/"$(basename "$file")".error --target-file "$tmp_path"/"$(basename "$file")".expanded.correct --output-file "$tmp_path"/"$(basename "$file")".error.tree
        python scripts/generate_eng_treebank/convert_tree.py --processes 20 --tokenizer "facebook/bart-large" --source-file "$tmp_path"/"$(basename "$file")".source --target-file "$tmp_path"/"$(basename "$file")".correct --output-file "$tmp_path"/"$(basename "$file")".source.tree
        python scripts/generate_eng_treebank/convert_tree.py --processes 20 --tokenizer "facebook/bart-large" --source-file "$tmp_path"/"$(basename "$file")".correct --target-file "$tmp_path"/"$(basename "$file")".correct --output-file "$tmp_path"/"$(basename "$file")".correct.tree
        python scripts/generate_eng_treebank/expand_correct_lines.py --input-file "$tmp_path"/"$(basename "$file")".source.tree --match-file "$tmp_path"/"$(basename "$file")".match --output-file "$tmp_path"/"$(basename "$file")".expanded.source.tree
    ...
    for portion in "${portions[@]}"; do
        ...
        python scripts/generate_eng_treebank/filter_illegal.py --input-file "$tmp_path"/silver."$portion".pid --output-file "$output_path"/"$portion".pid
    done
