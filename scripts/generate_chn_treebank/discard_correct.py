...
if __name__ == "__main__":
    ...
    correct_output = open(f"{args.output_file}.correct", "w")
    source_line = ""
    correct_line = ""
    ...
    with open(args.input_file, "r") as lines:
        for line in lines:
            ...
            if len(line) == 0:
                ...
                if correct_line == source_line:
                    correct_output.write(f"S\t{source_line}\n")
                    correct_output.write(f"T\t{correct_line}\n\n")
                elif 0 < len(correct_line) < 1.5 * len(source_line):
